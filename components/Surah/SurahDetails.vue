  ...
  <div class="surah-detail dark:text-gray-400">
    <base-header-card>
      ...
      <p class="text-sm md:text-base mb-2">
        Surah Number: {{ details.number }} / {{ details.revelation.id }} /
    ...
    <base-header-card>
      <input
        ...
        :placeholder="`Search for Ayah example: ${details.numberOfVerses}`"
        v-model="query"
      />
    </base-header-card>
          ...
          <div class="ulang-ayat-check">
            ...
            >
              <img
                ...
                class="w-5 md:mr-2"
                alt="repeat icon"/>
              <p>Repeat Audio <span class="text-xs">{{ isLooping ? '(active)' : '(non-active)' }}</span></p>
            </span>
          </div>
          <nuxt-link
            class="font-semibold cursor-pointer w-max hover:underline"
